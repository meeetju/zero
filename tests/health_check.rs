...
use uuid::Uuid;
use zero::configuration::{DatabaseSettings, get_configuration};
...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    if std::env::var("TEST_LOG").is_ok() {
        let subscriber = get_subscriber(subscriber_name, default_filter_level, std::io::stderr);
...
#[tokio::test]
async fn subscribe_returns_a_400_when_data_is_incorrect() {
    let app = spawn_app().await;
    let client = reqwest::Client::new();
    let test_cases = vec![
...
#[tokio::test]
async fn subscribe_returns_a_400_when_names_are_invalid() {
    let app = spawn_app().await;
